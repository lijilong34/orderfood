<template>
  <div class="order-list">
      ...
      <el-form-item label="桌号">
        <el-input v-model="tableNo" placeholder="桌号"/>
      </el-form-item>
      <el-form-item>
    ...
    <el-table-column label="操作"  width="150">
      <template #default="scope">
        ...
        <el-button type="primary" size="small" v-if="scope.row.status==1" @click="handleEdit(scope.row.id)" >
          已完成
        </el-button>
      </template>
    </el-table-column>
...
onMounted(() => {
  getOrderList(selectwhere.value); // 组件挂载后加载数据
});

// 获取订单列表
const getOrderList = async () => {
  try {
    let res = await orderInfo.orderInfo(selectwhere.value);
...
let res=await OrderApi.completedorder(order);
let order1=await OrderApi.selectorderbyid(id);
console.log("订单对象",order1)
let addmessage=await OrderApi.addmessage({
  userId:order1.data.entity.userId,
  type:2,
  title:'订单提醒',
  content:'您的订单已完成，请注意查收'
})
getOrderList();
};
// 查看
