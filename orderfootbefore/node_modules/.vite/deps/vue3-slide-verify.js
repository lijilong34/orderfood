import { Ft as onMounted, K as createCommentVNode, Kn as ref, Vt as openBlock, W as createBaseVNode, Wn as reactive, _r as normalizeStyle, hr as normalizeClass, jt as onBeforeUnmount, nt as defineComponent, q as createElementBlock, vr as toDisplayString } from "./vue.runtime.esm-bundler-7foM4kIy.js";

//#region node_modules/vue3-slide-verify/dist/vue3-slide-verify.es.js
var PI = Math.PI;
function sum(x, y) {
	return x + y;
}
...
function draw(ctx, x, y, l, r, operation) {
	ctx.beginPath();
	ctx.moveTo(x, y);
	ctx.arc(x + l / 2, y - r + 2, r, .72 * PI, 2.26 * PI);
	ctx.lineTo(x + l, y);
	ctx.arc(x + l + r - 2, y + l / 2, r, 1.21 * PI, 2.78 * PI);
	ctx.lineTo(x + l, y + l);
	ctx.lineTo(x, y + l);
	ctx.arc(x + r - 2, y + l / 2, r + .4, 2.76 * PI, 1.24 * PI, true);
	ctx.lineTo(x, y);
	ctx.lineWidth = 2;
...
function useSlideAction() {
	...
	const end = (e, cb) => {
		...
		if ((e instanceof MouseEvent ? e.clientX : e.changedTouches[0].pageX) === origin.x) return false;
...
var _sfc_main = defineComponent({
	...
	setup(props, { emit }) {
		...
		const reset = () => {
			...
			(_a = canvasCtx.value) == null || _a.clearRect(0, 0, w, h);
			(_b = blockCtx.value) == null || _b.clearRect(0, 0, w, h);
			block.value.width = w;
			img.src = getRandomImg(imgs);
		};
		const refresh = () => {
		...
		onMounted(() => {
			...
			img = createImg(imgs, () => {
				...
				const L = l + r * 2 + 3;
				if (offset > 0) if (offset >= L + 10 && offset <= w - (L + 10)) blockX.value = offset;
				else throw new Error(`offset must be greater than ${L + 10} and less than ${w - (L + 10)}`);
				else blockX.value = getRandomNumberByRange(L + 10, w - (L + 10));
...
var _hoisted_3 = { class: "slide-verify-slider-text" };
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
	return openBlock(), createElementBlock("div", {
		...
		createBaseVNode("div", { class: normalizeClass({ "slider-verify-loading": _ctx.loadBlock }) }, null, 2),
		...
		}, _cache[5] || (_cache[5] = [createBaseVNode("i", { class: "iconfont icon-refresh" }, null, -1)]))) : createCommentVNode("", true),
		createBaseVNode("canvas", {
			ref: "block",
			width: _ctx.w,
			height: _ctx.h,
		...
		createBaseVNode("div", { class: normalizeClass(["slide-verify-slider", {
		...
		}, [createBaseVNode("i", { class: normalizeClass([
			"slide-verify-slider-mask-item-icon",
			"iconfont",
			`icon-${_ctx.sliderBox.iconCls}`
		]) }, null, 2)], 36)], 4), createBaseVNode("span", _hoisted_3, toDisplayString(_ctx.sliderText), 1)], 2)
	], 4);
}
